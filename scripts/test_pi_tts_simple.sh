#!/bin/bash
# Simple test script for Piper TTS audio output on Pi
...
echo "=========================================="
echo "Testing Piper TTS Audio Output"
echo "=========================================="
echo ""
...
echo "Running TTS test on Pi..."
if [ -n "$PI_PASSWORD" ]; then
    sshpass -p "$PI_PASSWORD" ssh "${PI_USER}@${PI_HOST}" "bash -l -c 'cd ${PI_PATH} && poetry run python -c \"$TEST_SCRIPT\"'"
else
    ssh "${PI_USER}@${PI_HOST}" "bash -l -c 'cd ${PI_PATH} && poetry run python -c \"$TEST_SCRIPT\"'"
fi
